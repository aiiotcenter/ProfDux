class StatsView {
    ...
    renderOuterGridTableLayout() {
        ...
        const gradeStatsReviewContainer = document.querySelector(
    ...
    tweakGridCSS(structureWidth = 100) {
        let gridTableTemplateColumns = `auto auto 1fr repeat(${this.structureColumns}, auto) auto`;

        let minWidth =
            "100px 100px 200px" +
            ` ${this.structureColumns * structureWidth}px ` +
            "100px";
    ...
    renderRows() {
        if (this.quizObjectArray.length > 0) {
            ...
            this.quizObjectArray.forEach((student) =>
                this.renderGradeRow(student)
            );
        } else
    ...
    renderGradeRow(rowObject) {
        ...
        const userImageCell = document.createElement("li");
        userImageCell.className = "fixed100";
        userImageCell.innerHTML = `  <div class="grid-cell-userimage"> 
